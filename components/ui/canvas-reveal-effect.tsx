...
function normalizeDotMatrixColors(colors: number[][]): number[][] {
  const first = colors[0] ?? [0, 0, 0];

  if (colors.length === 2) {
    return [first, first, first, colors[1], colors[1], colors[1]];
  }

  if (colors.length === 3) {
    return [first, first, colors[1], colors[1], colors[2], colors[2]];
  }

  return [first, first, first, first, first, first];
}

function buildDotMatrixFragmentShader(params: {
  center: DotMatrixCenterAxis[];
  shaderSnippet: string;
}) {
  const centerX = params.center.includes("x")
    ? "st.x -= abs(floor((mod(u_resolution.x, u_total_size) - u_dot_size) * 0.5));"
    : "";
  const centerY = params.center.includes("y")
...
function prepareUniforms(params: {
  definitions: Uniforms;
  resolution: { width: number; height: number };
}) {
  const prepared: Record<string, { value: unknown; type?: string }> = {};

  for (const uniformName in params.definitions) {
    const uniform = params.definitions[uniformName];
  ...
  prepared["u_resolution"] = {
    value: new THREE.Vector2(
      params.resolution.width * 2,
      params.resolution.height * 2,
    ),
  };
...
const DotMatrix: React.FC<DotMatrixProps> = ({
  ...
  const uniforms = React.useMemo(() => {
    const colorsArray = normalizeDotMatrixColors(colors);
  ...
  return (
    <Shader
      source={buildDotMatrixFragmentShader({
        center,
